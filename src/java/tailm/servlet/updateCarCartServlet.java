 ...
 * @author DELL INC
 */
@WebServlet(name = "updateCarCartServlet", urlPatterns = {"/updateCarCartServlet"})
public class updateCarCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int quantityCar = Integer.parseInt(request.getParameter("quantity"));
        ...
        try {
            ...
            if (mapQuantityPossible != null) {
                if (mapQuantityPossible.containsKey(carID)) {
                    ...
                    if (quantityCar > quantityPossible) {
                        request.setAttribute("STOCK", "OUT_OF_STOCK. This car has only " + quantityPossible + " units left");
                        cart.getCartCar().get(carID).setQuantity(quantityPossible);
                    } else {
                        cart.getCartCar().get(carID).setQuantity(quantityCar);
                    }
                }
            }
